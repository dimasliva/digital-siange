<template>
    ...
    <SelecteModal v-if="openMaster" :devices="lists" @savemodal="getSubs" @closemodal="openMaster = false"/>
		...
		<div class="content">
			<table>
        	  ...
        	  <th></th>
				...
				<tbody>
					<tr v-for="(list, i) in sortedLists" :key="i">
						<td class="list_icon">
						</td>
						<td class="replay">
							<span>{{list.name}}</span>
						...
						<td class="playlist">
							...
							<br>
              <span v-if="list.nextScheduleName" class="number">
                {{` (Устанавливается ${list.nextScheduleName})`}}
              </span>
							<span style="color:red" v-if="list.lastError">
								Не удалось применить расписание: {{list.lastError.request.scheduleName}} 
...
import { getPlayers, updateTickers } from '@/api/players/func';

export default {
  components: { Header, ScreenModal, SelecteModal, CreateModal, WarningModal },
  name: 'Players',
	data: () => ({
		text_warning: "",
		lists: [],
  ...
  unmounted() {
    ...
    async getPlayers() {
        ...
        this.text_warning = "Ошибка сервера: Не удалось получить плееры"
        setTimeout(() => {
    ...
    async getSubs() {
		...
		async setTicker(content) {
			let res = await updateTickers(content)
			if(res.status !== 'ok') {
        this.openWarning = true
        this.text_warning = "Ошибка сервера: Не удалось получить плееры"
        setTimeout(() => {
