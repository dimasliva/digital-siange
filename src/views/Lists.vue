<template>
    ...
    <CreateModal v-if="openEditName" :item="clickedItem" :title="'Переименовать плейлист'" @savemodal="saveName" @closemodal="openEditName = false"/>
...
import {formatDate, formatAMPM, copySchedule, delSchedule} from "@/api/func"
  ...
  methods: {
    ...
    async getLists() {
      await fetch("/api/schedules/list").then(async res => {
        ...
        this.lists.map(val => {
          val.lastmod = formatDate(new Date(val.lastmod)) + ', ' + formatAMPM(new Date(val.lastmod)) 
        })
      })
    },
    toEditPage(id) {
